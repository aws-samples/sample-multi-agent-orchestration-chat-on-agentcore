...
- `docs/adr/goal-loop-integration.md` — per-message GoalLoop (SDK 1.8 upgrade + `legacy-peer-deps`, plugin-ordering + resume-guard AGENT_COMPLETE hazard, judge-span observability exclusion, Haiku judge default). Consult before changing the `plugins` array order in `agent.ts`, `SessionPersistenceHook.onAfterInvocation`, or `strands-span-kind-fixer.ts`.

 ...
 * Security checks:
 *   - stdio transport is not allowed (arbitrary OS command execution)
...
export function getEnabledMCPServers(config: MCPConfigFile): Array<{
  ...
  for (const { name, config: serverConfig } of servers) {
    if (serverConfig.transport === 'stdio') {
      throw new MCPConfigError(
        `stdio transport is not allowed in the Backend API (server: "${name}"). Use HTTP or SSE transport instead.`
      );
    }

...
// ---------------------------------------------------------------------------
// Salvage: legacy v1 / corrupted blobs
// ---------------------------------------------------------------------------

/**
 * Attempt to recover a wire block from a legacy or corrupted shape.
 *
 * This handles two known cases:
 *
 *   1. **SDK 0.1.x v1 blobs (no schemaVersion)** — these were written with
 *      `block.type` already present, so the input is already a valid
 *      `WireContentBlock` and we just narrow its type.
 *
 *   2. **Bug-window blobs (SDK 1.x before this codec landed)** — these
 *      were written via `JSON.stringify(messageWithContent)` which invoked
 *      SDK class `toJSON()` and emitted Bedrock Converse shapes such as
 *      `{ toolUse: {...} }` without `type`. We detect and salvage them.
 *
 * Returns `null` when no shape match — the caller logs and falls back.
 */
export function salvageLegacyContentBlock(raw: unknown): WireContentBlock | null {
  if (!raw || typeof raw !== 'object') return null;
  const obj = raw as Record<string, unknown>;

  // Case 1: already in v1/v2 wire shape with explicit `type`.
  if (typeof obj.type === 'string') {
    return obj as unknown as WireContentBlock;
  }

  // Case 2: bug-window — Bedrock Converse-style without `type`.
  if (obj.toolUse && typeof obj.toolUse === 'object') {
    const tu = obj.toolUse as Record<string, unknown>;
    if (typeof tu.name === 'string' && typeof tu.toolUseId === 'string') {
      return {
        type: 'toolUseBlock',
        name: tu.name,
        toolUseId: tu.toolUseId,
        input: (tu.input ?? {}) as WireToolUseBlock['input'],
      };
    }
  }

  if (obj.toolResult && typeof obj.toolResult === 'object') {
    const tr = obj.toolResult as Record<string, unknown>;
    if (typeof tr.toolUseId === 'string') {
      return {
        type: 'toolResultBlock',
        toolUseId: tr.toolUseId,
        status: (tr.status === 'error' ? 'error' : 'success') as WireToolResultBlock['status'],
        content: (tr.content ?? '') as WireToolResultBlock['content'],
      };
    }
  }

  // SDK 1.x toJSON() of TextBlock returns just `{ text }` (no wrapper).
  if (typeof obj.text === 'string') {
    return { type: 'textBlock', text: obj.text };
  }

  // SDK 1.x toJSON() of ImageBlock returns `{ image: { format, source: { bytes } } }`.
  if (obj.image && typeof obj.image === 'object') {
    const im = obj.image as { format?: string; source?: { bytes?: string | Uint8Array } };
    if (im.format && im.source) {
      ...
      return {
        type: 'imageBlock',
        format: im.format as WireImageBlock['format'],
        base64,
      };
    }
  }
